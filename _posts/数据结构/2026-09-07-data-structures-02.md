...
* 每个结点连接的**子结点数目**（分支的数目），我们称为结点的**度**（Degree），而各个结点度的最大值称为树的度。
...
```c
Node pre = NULL;  //这里我们需要一个pre来保存前继结点
void preOrderThreaded(Node root){   //前序遍历线索化函数
    if(root == NULL) return;

    if(root->left == NULL) {   //首先判断当前结点左边是否为NULL，如果是，那么指向上一个结点
        root->left = pre;   //指向前继节点
        root->leftTag = 1;  //记得修改标记
    }
    
    if(pre && pre->right == NULL) {  //然后是判断上一个结点的右边是否为NULL，如果是那么进行线索化，将上一个节点的右子树指向当前节点
        pre->right = root;
        pre->rightTag = 1;  //记得修改标记
    }
    
    pre = root;   //每遍历完一个，需要更新一下pre，表示前继结点
...
```c
Node insert(Node root, E element){   //root是要插入的树的根节点，element是要插入的数
    if(root){
        if(element < root->element )    //如果插入结点值小于当前结点，那么说明应该放到左边去
            root->left = insert(root->left, element);
        else if(element > root->element)    //如果插入结点值大于当前结点，那么说明应该放到右边去
...
Node find(Node root, E target){
    while (root) {
        if( target < root->element )    //如果要找的值比当前结点小，说明肯定在左边
            root = root->left;
        else if(target > root->element )   //如果要找的值比当前结点大，说明肯定在右边
            root = root->right;
        else
...
动画网站：https://www.cs.usfca.edu/~galles/visualization/AVLtree.html
...
Node leftRotation(Node root){  //左旋操作，实际上就是把左边结点拿上来  //RR型
...
Node rightRotation(Node root){  //LL型
...
Node leftRightRotation(Node root){   //LR型
    root->left = leftRotation(root->left);
    return rightRotation(root);
}

Node rightLeftRightRotation(Node root){   //RL型
...
Node insert(Node root, E element){
    if(root == NULL) {    //如果结点为NULL，说明找到了插入位置，直接创建新的就完事
        ...
        root->left = insert(root->left, element);
        //当root->left为NULL时，insert（）会创建新的节点并返回该节点，此时root->left就是新节点，继续往下运行
        if(getHeight(root->left) - getHeight(root->right) > 1) {   //插入完成之后，需要计算平衡因子，看看是否失衡
            if(element < root->left->element) //接着需要判断一下是插入了左子树的左边还是右边，如果是左边那边说明是LL，如果是右边那说明是LR
...
E delete(Heap heap){
    E max = heap->arr[1]；
    e = heap->arr[heap->size];
    heap->arr[heap->size--];
    int index = 1;
    while (index * 2 <= heap->size) {      //从上往下找,判断是否有左子树
        int child = index * 2;   //先找到左孩子
        //看看右孩子和左孩子哪个大，先选一个大的出来
        if(child < heap->size && heap->arr[child] < heap->arr[child + 1])//不用判断child=heap->size，如果等于，则就只有一个左子树
            child += 1;  //右子树大
        if(e >= heap->arr[child]) break; //如果孩子节点都小于e，则此时index就是要替换的位置
        else heap->arr[index] = heap->arr[child];  //如果最大的孩子节点大于e，将该最大孩子节点移到其父节点位置，保持最大堆性质
        index = child;   //再将index指向原本child的位置，继续向下
    }
