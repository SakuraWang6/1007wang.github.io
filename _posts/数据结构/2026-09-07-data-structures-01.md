...
现在有一个==从小到大排序==的数组，给你一个目标值`target`，现在请你找到这个值在数组中的对应下标，如果没有，请返回`-1`：
...
};//这里struct ListNode* 是这样一个类型的指针next，指向下一个节点
...
_Bool offerQueue(ArrayQueue queue, E element){
	int pos = (queue->rear + 1) % queue->capacity;   //找下一个插入位置：rear+1对长度取余，如果是数组的最后一个，则回到数组0号位置。
    if(pos == queue->front)   //找到的下一个插入位置与front比较，如果队尾下一个就是队首，那么说明已满  //如果rear与front指向同一个位置则是队空
        return 0;
    queue->rear=pos;
...
```
**值得注意的是，在出队后front指向的逻辑上是空的，但实际上数组并没有将元素清除，但是该元素在逻辑上已经不在queue中了，所以在打印queue时从front下一个位置开始打印**
...
int initQueue(LinkedQueue queue) {
    ...
    queue->front = queue->rear = head;   //一开始两个指针都是指向头结点的，表示队列为空
    return 1;
}
...
```
![[FJGL_[KYY[CF8)`2$~[Z2{7.png]]
![[4Z8{AV]~3)(RGVBFJF4M[L1.png]]
...
```c
void printQueue(LinkedQueue queue){//如果队列为空，这个是有错误的
    printf("<<< ");
    Node node = queue->front->next;//先找到头节点的下一节点，因为头节点逻辑上是空的
...
E pollQueue(LinkedQueue queue){
    E e = queue->front->next->element;//将队列中第一个元素取出来
    Node node = queue->front->next;//取出第一个节点，为后面释放内存
    queue->front->next = queue->front->next->next;  //直接让头结点指向下下个结点
    //如果待出队节点是最后一个节点，则会指向NULL
...
struct ListNode* deleteDuplicates(struct ListNode* head){
    ...
    //为什么不使用head遍历：使用head遍历最后指向NULL，无法返回原链表
...
==[题解（双指针/迭代）](https://leetcode.cn/problems/reverse-linked-list/solutions/2361282/206-fan-zhuan-lian-biao-shuang-zhi-zhen-r1jel/)==
...
        ListNode*cur=head,*pre=NULL;//创建两个指针pre指向前继节点，cur指向当前节点
        while(cur!=NULL){
            struct ListNode*tem；
            tem=cur->next;//暂存后继
            cur->next=pre;//修改当前节点指向，到前继节点
            pre=cur;//更新前继节点
            cur=tem;//更新当前节点，访问下一节点
        }
        return pre;
    }
};
...
    if(len == index) return head;  //如果len和index长度一样，那也没必要继续了
